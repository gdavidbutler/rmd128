...
rmd256mix(
 ...
 ,const unsigned char x[]
  ...
  rmd256_bt w[16]; /* precomputed message words */
  ...
  for (i = 0; i < 16; ++i)
    w[i] = (rmd256_bt)x[i * 4] | (rmd256_bt)x[i * 4 + 1] << 8
         | (rmd256_bt)x[i * 4 + 2] << 16 | (rmd256_bt)x[i * 4 + 3] << 24;
  for (i = 0; i < 8; ++i)
    t[i] = h[i];
  for (i = 0; i < 8; ++i) {
    base = (i & 1) << 2;
...
rmd256hkdf(
 ...
 ,unsigned int sl
  ...
  unsigned char i[64];
  unsigned char o[64];
  unsigned char t[RMD256_SZ];
  unsigned char t2[RMD256_SZ];
  unsigned int l;
  unsigned int n;
  ...
  for (n = 0, l = 0; l < sl;) {
    ...
    rmd256update(&c, i, sizeof (i));
    if (n > 1)
      rmd256update(&c, t, RMD256_SZ);
    rmd256update(&c, d, dl);
    rmd256update(&c, &b, 1);
    rmd256final(&c, t2);
    rmd256init(&c);
    rmd256update(&c, o, sizeof (o));
    rmd256update(&c, t2, RMD256_SZ);
    rmd256final(&c, t);
    for (j = 0; j < RMD256_SZ && l < sl; ++j, ++l)
      s[l] = t[j];
  }
